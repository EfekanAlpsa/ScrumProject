...
def register():
    ...
    c = conn.cursor()
    c.execute(
        """INSERT INTO users (name, surname, username, password, tr_id, phone, email, address, usertype)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (name, surname, username, password, tr_id, phone, email, address, usertype),
    )
    conn.commit()
...
def save_event():
    ...
    if not all([processing_date, start_time, event_time, event_type, description]):
        event_warning_label.config(
            text="Please Fill in the Information Completely", fg="red"
        )
        return
    ...
    c = conn.cursor()
    c.execute(
        """INSERT INTO events (processing_date, start_time, event_time, event_type, description, user_id)
                 VALUES (?, ?, ?, ?, ?, ?)""",
        (processing_date, start_time, event_time, event_type, description, 1),
...
username_label.pack()
username_entry = tk.Entry(login_screen, width=35)
username_entry.pack()

password_label = tk.Label(login_screen, text="Password:")
password_label.pack()
password_entry = tk.Entry(login_screen, show="*", width=35)
password_entry.pack()
...
name_label.pack()
name_entry = tk.Entry(register_screen, width=35)
name_entry.pack()

surname_label = tk.Label(register_screen, text="Surname:")
surname_label.pack()
surname_entry = tk.Entry(register_screen, width=35)
surname_entry.pack()

username_label = tk.Label(register_screen, text="Username:")
username_label.pack()
username_entry = tk.Entry(register_screen, width=35)
username_entry.pack()

password_label = tk.Label(register_screen, text="Password:")
password_label.pack()
password_entry = tk.Entry(register_screen, show="*", width=35)
password_entry.pack()

tr_id_label = tk.Label(register_screen, text="TR ID No:")
tr_id_label.pack()
tr_id_entry = tk.Entry(register_screen, width=35)
tr_id_entry.pack()

phone_label = tk.Label(register_screen, text="Phone:")
phone_label.pack()
phone_entry = tk.Entry(register_screen, width=35)
phone_entry.pack()

email_label = tk.Label(register_screen, text="Email:")
email_label.pack()
email_entry = tk.Entry(register_screen, width=35)
email_entry.pack()

address_label = tk.Label(register_screen, text="Address:")
address_label.pack()
address_entry = tk.Entry(register_screen, width=35)
address_entry.pack()
...
login_button = tk.Button(
    register_screen, text="Back to Login", command=show_login_screen
)
login_button.pack()
...
processing_date_entry = DateEntry(calendar_screen, width=35)
processing_date_entry.pack()

start_time_label = ttk.Label(calendar_screen, text="Event Start Time:")
start_time_label.pack()
start_time_entry = ttk.Entry(calendar_screen, width=35)
start_time_entry.pack()

pick_start_time_button = ttk.Button(
    calendar_screen, text="Pick Start Time", command=lambda: pick_time(start_time_entry)
)
pick_start_time_button.pack()

event_time_label = ttk.Label(calendar_screen, text="Event Time:")
event_time_label.pack()
event_time_entry = ttk.Entry(calendar_screen, width=35)
event_time_entry.pack()

pick_event_time_button = ttk.Button(
    calendar_screen, text="Pick Event Time", command=lambda: pick_time(event_time_entry)
)
pick_event_time_button.pack()

event_type_label = tk.Label(calendar_screen, text="Type of Event:")
event_type_label.pack()
event_type_entry = tk.Entry(calendar_screen, width=35)
